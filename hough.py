...
# 11.12 霍夫变换直线检测
img = cv2.imread("deep-learn.jpg", flags=1)  # flags=1 读取为彩色图像
...
for i in range(8):
    ...
    if (theta < (np.pi/4)) or (theta > (3*np.pi/4)):  # 直线与图像上下相交
        ...
        cv2.line(imgEdge, pt1, pt2, (0, 0, 255),5)  # 绘制直线
    else:  # 直线与图像左右相交
...
plt.subplot(131), plt.title("Origin"), plt.imshow(cv2.cvtColor(img, cv2.COLOR_RGB2BGR), cmap='gray'), plt.axis('off')
