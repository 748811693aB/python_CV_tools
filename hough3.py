...
lines = cv2.HoughLinesP(imgCanny, rho, theta, threshold,minLineLength=min_line_len, maxLineGap=max_line_gap)  # lines: (n,1,4)
for line in lines:
    x1, y1, x2, y2 = line[0]
    cv2.line(imgEdge, (x1,y1), (x2,y2), (255, 215, 0), 10)  # 绘制直线

plt.figure(figsize=(9, 5))

plt.subplot(133), plt.title("Hough"), plt.imshow(cv2.cvtColor(imgEdge, cv2.COLOR_RGB2BGR)), plt.axis('off')
cv2.imwrite("hough.jpg",imgEdge)
plt.tight_layout()
plt.show()
